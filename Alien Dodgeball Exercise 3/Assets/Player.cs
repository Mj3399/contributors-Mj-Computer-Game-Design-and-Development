...
public class Player : MonoBehaviour
{
    ...
    {
        var orbb = Instantiate(OrbPrefab, transform.position + transform.right, Quaternion.identity);
        Rigidbody2D oRB = orbb.GetComponent<Rigidbody2D>();
        oRB.velocity = OrbVelocity * transform.right;
    }
    ...
    void Manoeuvre()
    {
        ...
        var vec = new Vector2(hor, -ver);
        rb.AddForce(vec * EnginePower);
        rb.angularVelocity = RotateSpeed * (-1 * Input.GetAxis("Rotate"));
    }
